...
\begin{abstract}
Road-sign condition and content is critically important globally for drivers safety and critical infrastructure maintenance. Several groups have developed methods to detect and evaluate various sign metrics. Additionally, growing interest in autonomous driving requires input from road-signs for critical decision making. Data acquisition and cleaning is an ongoing challenge when training deep learning models, and is commonly a limiting factor of model performance. In sign recognition, this problem is exacerbated by the many different variations in typical signage between geographic locations, differences in sign conditions, and difficulty in collecting adequate quality data. To improve training datasets, we created an image generation pipeline using only free resources and Blender, a common 3D modeling software. To test our synthetic data, we trained both  YOLOv5 and DETR models with synthetic, real, and a combination of datasets and validated performance on real-world data. We found a significant improvement in both model's performance metrics when synthetic data is used to supplement real-world images during model training. Our results establish the validity of using synthetic data, generated using Blender, to improve sign detection and demonstrate the advantages of using synthetically generated datatsets for deep learning tasks.     
\end{abstract}
...
\section{Methods: Generating New Datasets}
...
Recall & 0.376 & 0.235 & \textbf{0.405}\\
Precision & 0.582 & 0.326 & \textbf{0.612} \\
...
Our results show that for both YOLOv5s and DETR, the models trained on the combined dataset of real-world and synthetic data (Model 3) achieved the highest mAP@0.5, mAP@0.5:0.95, and recall. This indicates a consistent and noticeable improvement in detection performance with the addition of synthetic data across both model architectures. While the YOLOv5s model trained solely on real images (Model 1) achieved the highest precision, DETR’s Model 1 also exhibited a relatively high precision. However, both saw slightly lower mAP and recall compared to their respective combined-data counterparts, suggesting more conservative detection behavior when trained on real data alone.

In contrast, the models trained only on synthetic data (Model 2) underperformed across most metrics in both YOLOv5s and DETR, especially in terms of mAP and recall. However, the synthetic-only DETR model retained a moderate precision score, reinforcing the observation that synthetic data can lead to accurate, albeit sparse, detections. These consistent trends across two very different architectures strengthen the conclusion that synthetic data alone is not sufficient for robust real-world performance, but becomes highly effective when used in conjunction with real data—enhancing viewpoint diversity, closing domain gaps, and substantially reducing the burden of data collection and annotation.
